...
import (
	...
	"github.com/rockcookies/go-gen/helper"
	"github.com/rockcookies/go-gen/internal/generate"
	"github.com/rockcookies/go-gen/internal/model"
	"github.com/rockcookies/go-gen/internal/parser"
	"github.com/rockcookies/go-gen/internal/utils/pools"
)
...
func (g *Generator) generateQueryFile() (err error) {
	...
	err = render(g.Tmpl.Header, &buf, map[string]interface{}{
	...
	if g.judgeMode(WithDefaultQuery) {
		err = render(g.Tmpl.DefaultQuery, &buf, g)
		if err != nil {
			return err
		}
	}
	err = render(g.Tmpl.QueryMethod, &buf, g)
	if err != nil {
		return err
	}
	...
	if g.WithUnitTest {
		...
		err = render(g.Tmpl.DIYMethodTestBasic, &buf, nil)
		if err != nil {
			return err
		}
		err = render(g.Tmpl.QueryMethodTest, &buf, g)
		if err != nil {
			g.db.Logger.Error(context.Background(), "generate query unit test fail: %s", err)
			return nil
		}
...
func (g *Generator) generateQueryUnitTestFile(data *genInfo) (err error) {
	...
	err = render(g.Tmpl.CRUDMethodTest, &buf, data.QueryStructMeta)
	if err != nil {
		return err
	}

	for _, method := range data.Interfaces {
		err = render(g.Tmpl.DIYMethodTest, &buf, method)
		if err != nil {
			return err
		}
	}
...
func (g *Generator) generateModelFile() error {
	...
	for _, data := range g.models {
		...
		go func(data *generate.QueryStructMeta) {
			...
			var buf bytes.Buffer
			err := render(g.Tmpl.Model, &buf, data)
			if err != nil {
				errChan <- err
				return
			}
			...
			for _, method := range data.ModelMethods {
				err = render(g.Tmpl.ModelMethod, &buf, method)
				if err != nil {
					errChan <- err
					return
				}
